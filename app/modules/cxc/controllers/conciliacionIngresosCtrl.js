...
app.controller('conciliacionIngresosCtrl', function ($scope, $state, cxcService, cxpService, serverConf, commonMethods, $filter) {
    ...
    function grillaConciliacion() {
        ...
        var rowTemplate = '<div style="height: 100%" ng-class="{red: (row.getProperty(\'conciliadoIngreso\') == \'A\'), yellow: (row.getProperty(\'conciliadoIngreso\') == \'M\' )}"><div ng-style="{ \'cursor\': row.cursor }" ng-repeat="col in renderedColumns" ng-class="col.colIndex()" class="ngCell ">' +
        ...
        $scope.gridConciliacion = {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    displayName: "Batch Name Ingresos",
                    width: '20%',
                    enableCellEdit: false,
                    headerClass: "header-center",
                    cellClass: "text-right",
                    cellEditableCondition: "row.getProperty(\'conciliadoIngreso\') != 'A'",
                    cellTemplate: '<input ng-class="{red: (row.getProperty(\'conciliadoIngreso\') == \'A\'), yellow: (row.getProperty(\'conciliadoIngreso\') == \'M\' )}" type="text" ng-click="$event.stopPropagation()" class="form-control text-right" style="font-size: 100%" ng-input="COL_FIELD" ' +
    ...
    $scope.disabledCampoBatchName = function (row) {
        ...
        var valor = false;
        switch (row.entity.conciliadoIngreso) {
            case 'A' : // PAGADOS
                valor = true;
                break;
            default :
                ...
                case $scope.gridConciliacion.$gridScope.gridId:
                    switch (columnaSeleccionada) {
                        case 'batchNameDebitoFiscal':
                            if (datosFila.conciliadoIngreso != 'A') {
                                if (datosFila.batchNameDebitoFiscal) {
                                    datosFila.conciliadoIngreso = "M";
                                } else {
                                    datosFila.conciliadoIngreso = "N";
                                }
                            }
                            break;
                    }
    ...
    $scope.guardaReferenciacionContable = function () {
        ...
        cxcService.putReferenciacionContable($scope.conciliacionPojoLista, {}, serverConf.ERPCONTA_WS, function (respuesta) {
            ...
            $scope.showCustomModal({
                ...
                bodyText: "Se registro exitosamente la referenciación.",
            ...
            $scope.showCustomModal({
                ...
                bodyText: "Existe un error al registrar la referenciación.",
                actionButtonText: "Continuar",
                type: 'error',
                closeAfter: 6000
            });
    ...
    $scope.exportacionXLS = function() {
        ...
        for(var i = 0; i < $scope.conciliacionPojoLista.length; i++) {
            ...
            for(var j = 0; j < columnDefs.length; j++) {
                ...
                } else if( field === 'conciliadoIngreso' ) {
                    switch(value){
        ...
        commonMethods.exportarXLS('Excel Mayor De Ingresos', xlsContent, true);
    };
